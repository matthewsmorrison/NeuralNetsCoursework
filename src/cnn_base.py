...
def cnn():
    ...
    cnn.add(Convolution2D(filters=64,kernel_size=(3,3), padding = 'same',strides = (1,1), input_shape = (width, height,1)))
    cnn.add(Activation('relu'))
    cnn.add(MaxPooling2D(pool_size=(2,2), strides = (2,2)))
    cnn.add(Convolution2D(filters=128,kernel_size=(5,5), padding = 'same',strides = (1,1)))
    cnn.add(Activation('relu'))
    cnn.add(MaxPooling2D(pool_size=(2,2), strides = (2,2)))
...
x_train = data["X_train"].reshape(data["X_train"].shape[0],height, width, depth)
y_train = np_utils.to_categorical(data["y_train"], classes)
x_val = data["X_val"].reshape(data["X_val"].shape[0],height, width, depth)
y_val = np_utils.to_categorical(data["y_val"], classes)
x_test = data["X_test"].reshape(data["X_test"].shape[0],height,width, depth)
y_test = np_utils.to_categorical(data["y_test"], classes)
