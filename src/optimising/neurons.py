...
numberOfNeurons = [100,250,500,750,1000]
...
for numberNeuron in numberOfNeurons:
    model = FullyConnectedNet([numberNeuron], input_dim = 48*48*1, num_classes=7, dropout=0, reg=0.5, dtype=np.float64, seed=450)
    number_epochs = 50
...
for i, loss in enumerate(losses):
    plt.plot(loss,'-o',ms=0.1,label="# of neurons: " + str(numberOfNeurons[i]))
plt.ylim(0, 6)
...
for i, accuracy in enumerate(accuracies):
    plt.plot(accuracy,'-o',ms=1, label="# of neurons: " + str(numberOfNeurons[i]))
...
xTicks = [str(neuron) for neuron in numberOfNeurons]
...
fig.savefig("src/optimising/outputs/numberofneurons.png")
