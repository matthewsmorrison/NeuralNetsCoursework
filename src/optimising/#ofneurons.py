...
numberoflayers = [[500],[500,500],[500,250],[1000,500]]
...
for layer in numberoflayers:
    model = FullyConnectedNet(layer, input_dim = 48*48*1, num_classes=7, dropout=0, reg=0.5, dtype=np.float64, seed=450)
    number_epochs = 100
...
for i, loss in enumerate(losses):
    plt.plot(loss,'-o',ms=0.1,label="architecture: " + str(numberoflayers[i]))
plt.ylim(0, 6)
...
for i, accuracy in enumerate(accuracies):
    plt.plot(accuracy,'-o',ms=1, label="architecture: " + str(numberoflayers[i]))
...
xTicks = [str(arch) for arch in numberoflayers]
...
fig.savefig("src/optimising/outputs/numberoflayers.png")
