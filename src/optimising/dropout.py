...
for eachDropout in dropouts:
    model = FullyConnectedNet([100], input_dim = 48*48*1, num_classes=7, dropout=eachDropout, reg=0, dtype=np.float64, seed=450)
    number_epochs = 50
...
for i, accuracy in enumerate(accuracies):
    plt.plot(accuracy,'-o',ms=1, label="dropout: " + str(dropouts[i]))
...
xTicks = [str(dropout) for dropout in dropouts]
