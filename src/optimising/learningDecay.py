...
for rateDecay in rateDecays:
    model = FullyConnectedNet([120], input_dim = 48*48*1, num_classes=7, dropout=0, reg=0, dtype=np.float64, seed=450)
    number_epochs = 50
...
for i, loss in enumerate(losses):
    plt.plot(loss,'-o',ms=0.1,label="learning decay: " + str(rateDecays[i]))
plt.ylim(0, 6)
...
for i, accuracy in enumerate(accuracies):
    plt.plot(accuracy,'-o',ms=1, label="learning decay: " + str(rateDecays[i]))
...
xTicks = [str(decay) for decay in rateDecays]
